...
export function IncomeStructureSection({ transactions, categories, accounts, banks, periodLabel }: Props) {
  ...
  }, [incomeTx, drillSubcategory])

  function handleRootClick(item: PieDataItem) {
    const cat = catMap.get(item.catId)
    if (!cat) return
    const subs = categories.filter((c) => c.parentId === cat.id && c.type === 'income')
  ...
  const headerText = drillCategory
    ? `Структура доходов: ${drillCategory.name}`
    : `Структура доходов за ${periodLabel}`
  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-sm">
      ...
      {drillSubcategory ? (
        ...
        <div className="text-center py-8 text-gray-400 text-sm">Нет доходов за этот период</div>
      )}
    </div>
  )
}